...
$this->title = $category->name.' - 常见问题 - 51前途网';
if ($category->alias == 'question') {
    $this->title = '提供UI设计培训机构,web前端培训等培训教程常见问题 - 51前途网';
}
$this->keywords = '';
$this->description = '51前途为你解决UI设计培训机构,web前端培训等培训教程常见问题，扫除就职障碍。';
?>
                        ...
                        <div class="box-bd">
                            <ol class="breadcrumbs">
                                <?php if (is_array($parents)) foreach ($parents as $parent) {?>
                                    <li>
                                        <?php if (count($parents) == 1) { $alias = 'question'; } else { $alias = ArrayHelper::getValue($parent,'alias');}?>
                                        <a href="/<?= $alias ?>"><?php if (ArrayHelper::getValue($parent,'level') == 1) { ?> 全部问题 <?php } else { echo ArrayHelper::getValue($parent,'name');}?> </a>
                                    </li>
                                <?php } ?>
                                <?php if($category->parent_id) {?><li><?= $category->name ?></li><?php } ?>
                            </ol>
                            <div class="category">
                                <label class="fl category-label">分部问题：</label>
                            ...
                            <div class="category-nav">
                                ...
                                <a class="orange" href="javascript:;"><?php if($category->parent_id) {?><?= $category->name ?><?php } else { echo "全部问题";}?></a>
                                <a href="javascript:;" class="fr orange cageogry-link">
                                    <i class="iconfont">&#xe7c0;</i>
                                </a>
                                <div class="category-box">
                                    <dl class="category-list">
                                        <?php if (array_key_exists('question',$oneCategory) && array_key_exists($oneCategory['question'],$twoCategory)) foreach ($twoCategory[$oneCategory['question']] as $two) {?>
                        ...
                        <div class="box-bd">
                            ...
                            <ul class="recommend-list">
                                <?php if (is_array($newsWords)) foreach ($newsWords as $newsWord) {?>
                                    <li class=" ov">
                                        <a href="/question/<?= ArrayHelper::getValue($newsWord,'id')?>"><img class="fl mr16" src="<?php if (ArrayHelper::getValue($newsWord,'thumb')) { echo ArrayHelper::getValue($newsWord,'thumb'); } else {?> /static/images/course268_180.png<?php }?>" alt=""></a>
                                        <h3 class="recommend-title fs-lg orange"><a href="/question/<?= ArrayHelper::getValue($newsWord,'id')?>"><?= ArrayHelper::getValue($newsWord,'title')?></a></h3>
