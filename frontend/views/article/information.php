...
$this->title = $category->name.' - 行业资讯 - 51前途网';
if ($category->alias == 'information') {
    $this->title = '最新最全的平面设计培训,淘宝美工教程等行业资讯 - 51前途网';
}
$this->keywords = '';
$this->description = '51前途网提供史上最新最全的平面设计培训,淘宝美工教程等行业资讯，掌握设计行业趋势。';
?>
              ...
              <div class="box-bd">
                <ol class="breadcrumbs">
                    <?php if (is_array($parents)) foreach ($parents as $parent) {?>
                        <li>
                            <?php if (count($parents) == 1) { $alias = 'information'; } else { $alias = ArrayHelper::getValue($parent,'alias');}?>
                            <a href="/<?= $alias ?>"><?php if (ArrayHelper::getValue($parent,'level') == 1) { ?> 全部资讯 <?php } else { echo ArrayHelper::getValue($parent,'name');}?> </a>
                        </li>
                    <?php } ?>
                    <?php if($category->parent_id) {?><li><?= $category->name ?></li><?php } ?>
                </ol>
                <div class="category">
                  <label class="fl category-label">分部资讯：</label>
                ...
                <div class="category-nav">
                  ...
                  <a class="orange" href="javascript:;"><?php if($category->parent_id) {?><?= $category->name ?><?php } else { echo "全部资讯";}?></a>
                  <a href="javascript:;" class="fr orange cageogry-link">
                    <i class="iconfont">&#xe7c0;</i>
                  </a>
                    <div class="category-box">
                        <dl class="category-list">
                            <?php if (array_key_exists('information',$oneCategory) && array_key_exists($oneCategory['information'],$twoCategory)) foreach ($twoCategory[$oneCategory['information']] as $two) {?>
            ...
            <div class="box">
              ...
              <div class="box-bd">
                ...
                <ul class="recommend-list">
                  ...
                  <li class=" ov">
                      <a href="/information/<?= ArrayHelper::getValue($newsWord,'id')?>"><img class="fl mr16" src="<?php if (ArrayHelper::getValue($newsWord,'thumb')) { echo ArrayHelper::getValue($newsWord,'thumb'); } else {?> /static/images/course268_180.png<?php }?>" alt="" ></a>
                      <h3 class="recommend-title fs-lg orange"><a href="/information/<?= ArrayHelper::getValue($newsWord,'id')?>"><?= ArrayHelper::getValue($newsWord,'title')?></a></h3>
