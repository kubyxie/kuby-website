...
class ArticleMetaTag extends \common\models\ArticleMeta
{
    ...
    {
        $result = self::find()->where(['key'=>$this->keyName])->where(['value'=>$tag])->asArray()->all();
    ...
    {
        $result = self::find()->where(['<>','value',''])->andWhere(['key'=>$this->keyName])->asArray()->limit($limit)->groupBy('value')->all();
        if( $result === null ) return [];
        return $result;
    }
}